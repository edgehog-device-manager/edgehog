...
const columns = [
  columnHelper.accessor("label", {
    header: () => (
      <FormattedMessage
        id="components.apps.containers.volumes-table.VolumesTable.label"
    ...
    meta: {
      ...
      getLink: (row, children) => (
        <Link
          ...
          params={{ volumeId: row.original.id }}
          className="row-link"
  ...
  columnHelper.accessor("driver", {
    header: () => (
      <FormattedMessage
        id="components.apps.containers.volumes-table.VolumesTable.driverTitle"
...
const VolumesTable = ({
  ...
  const volumes = useMemo<TableRecord[]>(() => {
    return compact(volumesFragment?.edges?.map((e) => e?.node)) ?? [];
  }, [volumesFragment]);
