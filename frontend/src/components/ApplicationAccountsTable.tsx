...
import Table, { createColumnHelper, type TableProps } from "./Table";
...
interface Props extends TableProps<TempAccount> {
  data: Array<TempAccount>;
}

const ApplicationAccountsTable: FunctionComponent<Props> = ({
  data,
  ...rest
}) => {
  const columnHelper = createColumnHelper<TempAccount>();
  const columns = [
    columnHelper.accessor("label", {
      header: () => (
        <FormattedMessage id="components.ApplicationAccountsTable.labelTitle" />
      ),
      cell: ({ getValue }) => <span className="text-nowrap">{getValue()}</span>,
    }),
    columnHelper.accessor("username", {
      header: () => (
        <FormattedMessage id="components.ApplicationAccountsTable.usernameTitle" />
      ),
      cell: ({ getValue }) => <span className="text-nowrap">{getValue()}</span>,
    }),
    // TODO add field enabled, others for future use
  ];
  return <Table {...rest} columns={columns} data={data} />;
};

ApplicationAccountsTable.displayName = "ApplicationAccountsTable";

export default ApplicationAccountsTable;
