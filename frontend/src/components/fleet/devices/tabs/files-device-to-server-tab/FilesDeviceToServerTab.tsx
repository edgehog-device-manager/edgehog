  ...
  const handleSubmit = useCallback(
    async (values: ManualFileUploadRequestData) => {
      if (!isOnline) {
        setErrorFeedback(
          <FormattedMessage
            id="components.fleet.devices.tabs.files-device-to-server-tab.FilesDeviceToServerTab.deviceOfflineError"
      ...
      try {
        ...
        const message =
          error instanceof Error
            ? error.message
            : intl.formatMessage({
                id: "components.fleet.devices.tabs.files-device-to-server-tab.FilesDeviceToServerTab.error.unknownError",
  ...
  const sourceTypeOptions = useMemo<SourceTypeOption[]>(() => {
    ...
    if (capabilities.storage != null) {
      options.push({
        ...
        label: intl.formatMessage({
          id: "components.fleet.devices.tabs.files-device-to-server-tab.FilesDeviceToServerTab.sourceType.storage",
    ...
    if (capabilities.filesystem != null) {
      options.push({
        value: "FILESYSTEM",
        label: intl.formatMessage({
          id: "components.fleet.devices.tabs.files-device-to-server-tab.FilesDeviceToServerTab.sourceType.filesystem",
  ...
  const content = (
    <>
      ...
      <Card className="gap-2 border-0 shadow-sm flex-grow-1 p-4">
        <h5>
          <FormattedMessage
            id="components.fleet.devices.tabs.files-device-to-server-tab.FilesDeviceToServerTab.requestHistory"
  ...
  return (
    <Tab
      ...
      title={intl.formatMessage({
        id: "components.fleet.devices.tabs.files-device-to-server-tab.FilesDeviceToServerTab.title",
    ...
    >
      <div className="mt-3">
        <h6>
          <FormattedMessage
            id="components.fleet.devices.tabs.files-device-to-server-tab.FilesDeviceToServerTab.uploadSource"
