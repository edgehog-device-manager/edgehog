...
const columns = [
  columnHelper.accessor("essid", {
    header: () => (
      <FormattedMessage
        id="components.fleet.devices.wifi-scan-results-table.WiFiScanResultsTable.apEssidTitle"
  ...
  columnHelper.accessor("channel", {
    header: () => (
      <FormattedMessage
        id="components.fleet.devices.wifi-scan-results-table.WiFiScanResultsTable.apChannelTitle"
  ...
  columnHelper.accessor("macAddress", {
    header: () => (
      <FormattedMessage
        id="components.fleet.devices.wifi-scan-results-table.WiFiScanResultsTable.apMacAddressTitle"
  ...
  columnHelper.accessor("rssi", {
    header: () => (
      <FormattedMessage
        id="components.fleet.devices.wifi-scan-results-table.WiFiScanResultsTable.apRssiTitle"
        defaultMessage="RSSI"
      />
    ),
    cell: ({ getValue }) => {
  ...
  columnHelper.accessor("seenAt", {
    header: () => (
      <FormattedMessage
        id="components.fleet.devices.wifi-scan-results-table.WiFiScanResultsTable.seenAtTitle"
...
const WiFiScanResultsTable = ({ className, deviceRef }: Props) => {
  ...
  if (!data.wifiScanResults || !data.wifiScanResults.length) {
    return (
      <Result.EmptyList
        title={
          <FormattedMessage
            id="components.fleet.devices.wifi-scan-results-table.WiFiScanResultsTable.noResults.title"
            defaultMessage="No results"
          />
        }
      >
        <FormattedMessage
          id="components.fleet.devices.wifi-scan-results-table.WiFiScanResultsTable.noResults.message"
  ...
  const sorted = data.wifiScanResults
    .map((scanResult) => {
    ...
    .sort((scan1, scan2) => scan2.seenAt.getTime() - scan1.seenAt.getTime());

  const firstConnectedIdx = sorted.findIndex((r) => r.connected);
  const wifiScanResults = sorted.map((scanResult, idx) =>
    scanResult.connected && idx !== firstConnectedIdx
