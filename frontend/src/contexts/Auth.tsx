...
const hasRequiredValues = (session?: Session | null): session is Session =>
  session != null && session.tenantSlug != null && session.authToken != null;
...
const AuthProvider = ({ children, fetchGraphQL }: AuthProviderProps) => {
  ...
  const updateSession = useCallback(
    (newSession: Session | null, persistConfig: boolean = false) => {
      saveSession(newSession, persistConfig);
    },
    [saveSession],
  );

  const validateSession = useCallback(
    (session: Session | null): Promise<boolean> => {
      if (!hasRequiredValues(session)) {
        return Promise.resolve(false);
      }
      return fetchGraphQL(authQuery, {}, session)
