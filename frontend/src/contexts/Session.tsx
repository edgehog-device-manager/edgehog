...
// Use null to signal that the context has not been provided.
const SessionContext = createContext<SessionContextValue | null>(null);

// A default session is created if none exists.
const defaultSession: Session = { tenantSlug: "", authToken: null };
...
const SessionProvider = ({ children }: { children: React.ReactNode }) => {
  // Always ensure that session is never null.
  ...
  const updateSession = useCallback(
    (sessionData: Session, persist: boolean = false) => {
      saveSession(sessionData.authToken ? sessionData : null, persist);
