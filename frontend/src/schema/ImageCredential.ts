...
import {
  ...
  ImageCredential,
  ImageCredentialResult,
} from "types/ImageCredential";
...
export const imageCredentialResultSchema: SchemaOf<ImageCredentialResult> =
  object({
    id: string().required(),
  }).required();

export const imageCredentialSchema: SchemaOf<ImageCredential> = object()
  .concat(commonImageCredentialSchema)
  .concat(imageCredentialResultSchema)
  .required();

export const createImageCredentialSchema: SchemaOf<CreateImageCredential> =
  object({
    password: string()
      .required()
