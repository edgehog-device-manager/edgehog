...
defmodule Edgehog.CampaignsFixtures do
  ...
  defp deploy_for_required_operations(mechanism_type, release_id, channel_id, tenant) do
    if mechanism_type in [:deployment_start, :deployment_stop, :deployment_delete] do
  ...
  defp get_release_for_target(target, mechanism_type, tenant) do
    campaign = Core.get_campaign!(tenant.tenant_id, target.campaign_id)
  ...
  defp terminated_target_fixture(opts, status) do
    ...
    case status do
      :failure -> Core.mark_target_as_failed!(target, now)
      :success -> Core.mark_target_as_successful!(target, now)
    end
  end
