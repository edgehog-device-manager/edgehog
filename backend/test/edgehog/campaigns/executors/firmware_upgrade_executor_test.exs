...
defmodule Edgehog.Campaigns.Executors.FirmwareUpgradeExecutorTest do
  ...
  describe "Executor immediately terminates" do
    ...
    test "when firmware upgrade campaign is already marked as failed", %{tenant: tenant} do
      ...
      [target] = update_campaign.campaign_targets
  ...
  describe "Executor sends" do
    test "all target OTA Requests in parallel if there are enough available slots", %{
      ...
      parent = self()
      ref = make_ref()
      base_image_url = update_campaign.campaign_mechanism.value.base_image.url
      target_device_ids = Enum.map(update_campaign.campaign_targets, & &1.device.device_id)
    ...
    test "OTA Requests only to online targets", %{tenant: tenant} do
      ...
      {offline_targets, online_targets} =
        Enum.split(update_campaign.campaign_targets, offline_count)
  ...
  defp assert_update_campaign_outcome(tenant, id, outcome) do
    update_campaign = Core.get_campaign!(tenant.tenant_id, id)
