...
defmodule Edgehog.Campaigns.Executors.DeploymentDeleteExecutorTest do
  ...
  describe "Executor immediately terminates" do
    test "when a deployment delete campaign has no targets", %{tenant: tenant} do
      deployment_campaign = campaign_fixture(tenant: tenant, mechanism_type: :deployment_delete)
    ...
    test "when deployment delete campaign is already marked as failed", %{tenant: tenant} do
      ...
      [target] = deployment_campaign.campaign_targets
  ...
  describe "Executor sends" do
    test "all target Delete Deployment Requests in parallel if there are enough available slots",
      ...
      parent = self()
      ref = make_ref()
      target_device_ids = Enum.map(deployment_campaign.campaign_targets, & &1.device.device_id)
    ...
    test "Delete Deployment Requests only to online targets", %{tenant: tenant} do
      ...
      {offline_targets, online_targets} =
        Enum.split(deployment_campaign.campaign_targets, offline_count)
  ...
  describe "Executor marks deployment delete campaign as successful" do
    ...
    test "if all targets are successful", ctx do
      ...
      trigger_destroy_for_pending_deployments(tenant, deployment_campaign_id)
  ...
  defp trigger_destroy_for_pending_deployments(tenant, deployment_campaign_id) do
    tenant.tenant_id
    |> Core.list_in_progress_targets(deployment_campaign_id)
    |> Enum.each(fn target ->
      trigger_destroy_and_gc!(tenant, target.deployment_id)
  ...
  defp assert_deployment_campaign_outcome(tenant, id, outcome) do
    deployment_campaign = Core.get_campaign!(tenant.tenant_id, id)
