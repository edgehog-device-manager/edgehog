...
defmodule Edgehog.Campaigns.Executors.DeploymentStartExecutorTest do
  ...
  describe "pause and resume deployment start executor" do
    test "pause suppresses new start requests and resume restarts rollout", %{tenant: tenant} do
      ...
      # Mark one pending deployment as running (freeing a slot)
      %{tenant_id: tenant_id, id: campaign_id} = campaign

      [target | _] =
        MechanismCore.list_in_progress_targets(%DeploymentStart{}, tenant_id, campaign_id)
      ...
      # Now resume, expect a new start request (to fill freed slot)
      resume_ref = expect_deployment_start_requests_and_send_sync(1)
    ...
    test "campaign can complete while paused", %{tenant: tenant} do
      ...
      # Mark all pending as running; should terminate with success while paused
      %DeploymentStart{}
