...
defmodule Edgehog.Campaigns.Executors.DeploymentStartExecutorTest do
  ...
  describe "Executor immediately terminates" do
    test "when a deployment start campaign has no targets", %{tenant: tenant} do
      deployment_campaign = campaign_fixture(tenant: tenant, mechanism_type: :deployment_start)
  ...
  describe "Executor marks deployment start campaign as successful" do
    ...
    test "if all targets are successful", ctx do
      ...
      mark_all_pending_deployments_with_state(tenant, deployment_campaign_id, :started)
