...
defmodule Edgehog.Containers.DeviceRequest.Deployment.Provisioner.CoreTest do
  @moduledoc """
  Tests for the device request deployment provisioner Core.
  """
  ...
  describe "DeviceRequest deployment provisioner Core" do
    setup do
      ...
      [device_request_deployment] =
        deployment
        |> Ash.load!(
          [container_deployments: [device_request_deployments: [:device_request, :device]]],
          tenant: tenant
        )
    ...
    test "topic/1 returns the topic on which readiness is broadcast", _context do
      assert Core.topic(%{id: "device-request-deployment-id"}) ==
               "ready:device_request_deployments:device-request-deployment-id"

      assert Core.topic("device-request-deployment-id") ==
               "ready:device_request_deployments:device-request-deployment-id"
    end

    test "subscribe_topic/1 returns the topic the provisioner subscribes to", _context do
      assert Core.subscribe_topic(%{id: "device-request-deployment-id"}) ==
               "device_request_deployments:device-request-deployment-id"

      assert Core.subscribe_topic("device-request-deployment-id") ==
               "device_request_deployments:device-request-deployment-id"
    end

    test "name/1 returns the via tuple used to register the provisioner", _context do
      assert Core.name(%{id: "device-request-deployment-id"}) ==
               {:via, Registry,
                {Edgehog.Containers.DeviceRequest.Deployment.Provisioner.Registry,
                 "device-request-deployment-id"}}
    end
    ...
    test "send_to_device/2 sends the create request to the device", context do
      ...
      expect(CreateDeviceRequestRequest, :send_create_device_request_request, fn _, _, data ->
        assert data.id == device_request_deployment.device_request.id
      ...
      assert :ok ==
               Core.send_to_device(device_request_deployment,
                 tenant: tenant,
                 deployment: deployment
               )
    end
    ...
    test "reconcile/2 updates the resource when the status is reported by the device", context do
      %{device_request_deployment: device_request_deployment, tenant: tenant} = context

      expect(AvailableDeviceRequests, :get, fn _client, _device_id ->
        {:ok,
         [
           %DeviceRequestStatus{
             id: device_request_deployment.device_request.id,
             present: true
           }
         ]}
      end)

      assert {:ok, updated} = Core.reconcile(device_request_deployment, tenant: tenant)
      assert updated.id == device_request_deployment.id
