...
defmodule EdgehogWeb.Schema.Mutation.UpdateUpdateCampaignTest do
  ...
  describe "updateUpdateCampaign mutation" do
    ...
    test "updates the update campaign with valid data", %{
      ...
      updated_update_campaign = response["data"]["updateUpdateCampaign"]["updateCampaign"]
      assert updated_update_campaign["name"] == update_data.name

      assert updated_update_campaign["rolloutMechanism"]["maxInProgressUpdates"] ==
               update_data.rollout_mechanism.push.max_in_progress_updates
    end
  end

  @query """
  mutation UpdateUpdateCampaign($input: UpdateUpdateCampaignInput!) {
    updateUpdateCampaign(input: $input) {
      updateCampaign {
        name
        status
        outcome
