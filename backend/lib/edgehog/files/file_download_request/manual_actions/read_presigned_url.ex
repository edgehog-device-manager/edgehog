...
defmodule Edgehog.Files.FileDownloadRequest.ManualActions.ReadPresignedUrl do
  ...
  def run(input, _opts, context) do
    read_resource(
      ...
      input.arguments.file_download_request_id,
      input.arguments.filename
    )
  end

  @doc """
  Generate presigned URLs to download a file via HTTP requests.
  """
  def read_resource(tenant_id, file_download_request_id, filename) do
    ...
    with {:ok, presigned_url_get} <-
           ExAws.S3.presigned_url(public_s3_config, :get, bucket, file_path, expires_in: @presign_expiration_seconds) do
      {:ok, %{get_url: presigned_url_get}}
    end
  end
end
