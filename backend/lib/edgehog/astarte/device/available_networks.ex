...
defmodule Edgehog.Astarte.Device.AvailableNetworks do
  @moduledoc false

  @behaviour Edgehog.Astarte.Device.AvailableNetworks.Behaviour
  ...
  def get(%AppEngine{} = client, device_id) do
    with {:ok, %{"data" => data}} <-
      ...
      networks = Enum.map(data, &parse_available_networks/1)

      {:ok, networks}
    end
  end

  defp parse_available_networks({network_id, parameters}) do
    %NetworkStatus{
      id: network_id,
