...
defmodule Edgehog.Astarte.Device.AvailableDeviceRequests do
  @moduledoc false
  @behaviour Edgehog.Astarte.Device.AvailableDeviceRequests.Behaviour
  ...
  def get(%AppEngine{} = client, device_id) do
    with {:ok, %{"data" => data}} <-
      ...
      device_requests = Enum.map(data, &parse_device_request_properties/1)

      {:ok, device_requests}
    end
  end

  defp parse_device_request_properties({device_request_id, properties}) do
    %DeviceRequestStatus{
