...
defmodule Edgehog.Astarte.Device.AvailableFileBinds do
  @moduledoc false
  @behaviour Edgehog.Astarte.Device.AvailableFileBinds.Behaviour
  ...
  def get(%AppEngine{} = client, device_id) do
    with {:ok, %{"data" => data}} <-
      ...
      file_binds = Enum.map(data, &parse_file_bind_properties/1)

      {:ok, file_binds}
    end
  end

  defp parse_file_bind_properties({bind_id, properties}) do
    %FileBindStatus{
      id: bind_id,
      target_id: properties["targetId"],
      deployment_id: properties["deploymentId"],
      target_type: properties["targetType"],
