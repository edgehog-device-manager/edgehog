...
defmodule Edgehog.Campaigns.Executors.Lazy.LazyBatch do
  ...
  def handle_event(:internal, :init_data, :initialization, data) do
    %Data{campaign_id: campaign_id, tenant_id: tenant_id} = data
    ...
    # available_slots.
    campaign = Core.get_campaign!(tenant_id, campaign_id)

    mechanism =
      CampaignMechanism.Core.get_mechanism(campaign.campaign_mechanism.value, campaign)

    campaign_status = Core.get_campaign_status(campaign)

    # Early exit if campaign is already finished
    if campaign_status == :finished do
      {:stop, :normal}
    else
      target_count = Core.get_target_count(tenant_id, campaign_id)
      ...
      case campaign_status do
        :idle ->
          ...
          _ = Core.mark_campaign_in_progress!(campaign)
  ...
  def handle_event(:internal, :resume_campaign, :initialization, data) do
    ...
    failed_count = Core.get_failed_target_count(tenant_id, campaign_id)
    in_progress_count = Core.get_in_progress_target_count(tenant_id, campaign_id)
    available_slots = Core.available_slots(mechanism, in_progress_count)
    ...
    timeout_actions =
      tenant_id
      |> Core.list_in_progress_targets(campaign_id)
      |> Enum.map(fn in_progress_target ->
        operation_id = CampaignMechanism.Core.get_operation_id(mechanism, in_progress_target)

        # Side effect: receive updates for the target so we can track it
        CampaignMechanism.Core.subscribe_to_operation_updates!(mechanism, operation_id)
  ...
  def handle_event(:internal, :start_campaign, :initialization, data) do
    ...
    available_slots = Core.available_slots(mechanism, 0)
  ...
  def handle_event(:internal, :fetch_next_target, :execution, data) do
    ...
    case CampaignMechanism.Core.fetch_next_valid_target(mechanism, campaign_id, tenant_id) do
      {:ok, target} ->
      ...
      {:error, %Ash.Error.Invalid{errors: [%Ash.Error.Query.NotFound{}]}} ->
        ...
        cond do
          ...
          Core.has_idle_targets?(tenant_id, campaign_id) ->
  ...
  def handle_event(:internal, {:execute_on_target, target}, :execution, data) do
    ...
    case CampaignMechanism.Core.do_operation(mechanism, target) do
  ...
  def handle_event(:internal, {:operation_started, target}, :execution, data) do
    ...
    operation_id = CampaignMechanism.Core.get_operation_id(mechanism, target)
    CampaignMechanism.Core.subscribe_to_operation_updates!(mechanism, operation_id)
  ...
  def handle_event(:enter, _old_state, :campaign_success, data) do
    ...
    _ =
      tenant_id
      |> Core.get_campaign!(campaign_id)
      |> Core.mark_campaign_as_successful!()
  ...
  def handle_event(:internal, {:operation_success, operation}, _state, data) do
    ...
    _ =
      tenant_id
      |> Core.get_target_for_operation!(campaign_id, operation.device_id)
      |> Core.mark_target_as_successful!()

    # Unsubscribe from operation updates since it's completed
    CampaignMechanism.Core.unsubscribe_to_operation_updates!(mechanism, operation.id)
  ...
  def handle_event(:internal, {:operation_failure_event, operation}, state, data) do
    ...
    # TODO: decide
    # Core.format_operation_failure_log(operation, campaign_data)
  ...
  def handle_event({:timeout, {:retry, _operation_id}}, target_id, _state, data) do
    ...
    target = Core.get_target!(tenant_id, target_id)

    if Core.can_retry?(target, mechanism) do
  ...
  def handle_event(:internal, {:retry_target, target}, _state, data) do
    ...
    # TODO: move all update_target_latest_attempt to core
    target =
      target
      |> Core.increase_retry_count!()
      |> Edgehog.Campaigns.update_target_latest_attempt!(DateTime.utc_now())

    case CampaignMechanism.Core.retry_operation(mechanism, target) do
      :ok ->
  ...
  def handle_event(:internal, {:retry_threshold_exceeded, target}, _state, data) do
    ...
    operation_id = CampaignMechanism.Core.get_operation_id(mechanism, target)
    _ = CampaignMechanism.Core.mark_operation_as_timed_out!(mechanism, operation_id, tenant_id)

    # Unsubscribe from operation updates since we've given up on retries
    CampaignMechanism.Core.unsubscribe_to_operation_updates!(mechanism, operation_id)
  ...
  def setup_retry_timeout(tenant_id, target, mechanism) do
    ...
    operation_id = CampaignMechanism.Core.get_operation_id(mechanism, target)
    timeout_ms = Core.pending_request_timeout_ms(target, mechanism)
  ...
  defmacro __using__(_opts) do
    quote location: :keep do
      ...
      alias Edgehog.Campaigns.Executors.Lazy.LazyBatch, as: Data
      ...
      defdelegate setup_retry_timeout(tenant_id, target, mechanism),
        to: Data
