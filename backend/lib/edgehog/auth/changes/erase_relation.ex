...
defmodule Edgehog.Auth.Changes.EraseRelation do
  @moduledoc """
  ...
  def change(changeset, opts, _context) do
    Ash.Changeset.after_transaction(changeset, &erase_rel_tuple(&1, &2, opts))
  end

  defp erase_rel_tuple(_changeset, {:ok, result}, opts) do
    ...
    with {:ok, _} <- FGAService.delete(subj, rel, obj) do
      {:ok, result}
    end
  end

  defp erase_rel_tuple(_changeset, error, opts) do
    Logger.debug("Error while executing DB transaction. Skipping removing tuple on the provider.",
      error: error,
      opts: opts
    )
