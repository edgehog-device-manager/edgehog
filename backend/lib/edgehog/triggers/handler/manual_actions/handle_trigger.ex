...
defmodule Edgehog.Triggers.Handler.ManualActions.HandleTrigger do
  ...
  alias Edgehog.Triggers.DeviceDeleted
  ...
  defp handle_event(%DeviceDeleted{}, tenant, realm_id, device_id, _timestamp) do
    ...
    # Devices
    with {:ok, device} <- Ash.read_one(device_query, tenant: tenant) do
      Ash.destroy(device)
    end
  end
