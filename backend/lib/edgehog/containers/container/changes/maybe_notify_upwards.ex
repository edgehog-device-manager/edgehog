...
defmodule Edgehog.Containers.Container.Changes.MaybeNotifyUpwards do
  ...
  def change(changeset, _opts, _context) do
    ...
    |> Ash.Changeset.load(:deployments)
    |> Ash.Changeset.after_action(fn _changeset, container_deployment ->
      if container_deployment.is_ready,
        do:
          container_deployment
          |> Map.get(:deployments, [])
          |> Enum.reduce_while({:ok, container_deployment}, fn deployment, {:ok, container_deployment} ->
            case maybe_notify_upwards(deployment) do
  ...
  defp maybe_notify_upwards(deployment) do
    deployment
