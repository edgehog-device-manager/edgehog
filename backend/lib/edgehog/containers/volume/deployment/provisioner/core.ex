...
defmodule Edgehog.Containers.Volume.Deployment.Provisioner.Core do
  ...
  @impl Edgehog.Containers.Provisioner.Core.Behaviour
  def topic(%{id: id}), do: "ready:volume_deployments:#{id}"
  def topic(id), do: "ready:volume_deployments:#{id}"

  @impl Edgehog.Containers.Provisioner.Core.Behaviour
  def subscribe_topic(%{id: id}), do: "volume_deployments:#{id}"
  def subscribe_topic(id), do: "volume_deployments:#{id}"

  @impl Edgehog.Containers.Provisioner.Core.Behaviour
  def name(%{id: id}),
    do: {:via, Registry, {Edgehog.Containers.Volume.Deployment.Provisioner.Registry, id}}
  ...
  def send_to_device(resource, opts) do
    ...
    with {:ok, resource} <- Ash.load(resource, [:volume, :device], tenant: tenant),
         {:ok, actual_resource} <- Map.fetch(resource, :volume),
  ...
  def reconcile(resource, opts) do
    ...
    with {:ok, resource} <-
           Ash.load(resource, [volume: [], device: [:available_volumes]], tenant: tenant),
         {:ok, actual_resource} <- Map.fetch(resource, :volume),
         {:ok, device} <- Map.fetch(resource, :device),
         {:ok, available_resources} <- Map.fetch(device, :available_volumes) do
      available_resources
  ...
  def maybe_update(%VolumeStatus{created: created}, resource, tenant) do
    action =
      if created,
