...
defmodule Edgehog.Containers.Volume.Deployment.Provisioner.Core do
  @moduledoc """
  Volume provisioner core functions.
  ...
  def send_to_device(volume_deployment, opts) do
  ...
  @doc """
  Tries to reconcile the volume deployment with the property set by the device.

  The device publishes the available volumes, this function reads such property
  and either finds a state, and sets the volume deployment to that state or does
  not find a valid state, therefore the device does not have such volume
  deployed, and the function returns :not_found

  Alternatively, if something went wrong while updating the volume, an 
  `{:error, _}` is returned.

  Example:
  ```elixir
  Core.reconcile(volume_deployment, tenant: tenant)
  > {:ok, new_volume_deployment}

  Core.reconcile(volume_deployment, tenant: tenant)
  > :not_found
  ```
  """
  def reconcile(volume_deployment, opts) do
    tenant = Keyword.fetch!(opts, :tenant)

    with {:ok, volume_deployment} <-
           Ash.load(volume_deployment, [volume: [], device: [:available_volumes]], tenant: tenant),
         {:ok, volume} <- Map.fetch(volume_deployment, :volume),
         {:ok, device} <- Map.fetch(volume_deployment, :device),
         {:ok, available_volumes} <- Map.fetch(device, :available_volumes) do
      available_volumes
      |> Enum.find(:not_found, &(&1.id == volume.id))
      |> maybe_update(volume_deployment, tenant)
    end
  end

  defp maybe_update(%VolumeStatus{created: created}, volume_deployment, tenant) do
  ...
  defp maybe_update(other, _volume_deployment, _tenant), do: other
  ...
  - pan       :: the pan component is there to ensure a minimum timeout is
                 guaranteed. The pan is only applied when the volume deployment
