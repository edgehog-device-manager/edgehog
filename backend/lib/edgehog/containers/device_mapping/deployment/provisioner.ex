...
defmodule Edgehog.Containers.DeviceMapping.Deployment.Provisioner do
  ...
  # Send the device mapping if the number of retries does not exceed the max number of
  # retries.
  ...
  # Sends the device mapping deployment, without asking any questions. To check for
  ...
  defp maybe_update_state_on_send(error, state) do
    ...
    Logger.warning(
      "Error while sending the device mapping deployment #{id}: #{inspect(error)}. The operation will be retried shortly."
    )
