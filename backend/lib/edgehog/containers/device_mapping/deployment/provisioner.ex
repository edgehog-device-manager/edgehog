...
defmodule Edgehog.Containers.DeviceMapping.Deployment.Provisioner do
  ...
  emits a `ready:device_mapping_deployments:id` event whenever device mapping is present in the
  device.

  The provisioning flow can be described as follows:

  - start_link/1 called, init the process
  - The server subscribes to events on 'device_mapping_deployments:id' (id of the device mapping
    deployment)
  - if the device mapping deployment is already ready, the provisioner terminates
  ...
  - Astarte triggers update the device mapping deployment state, marking it as present or
  ...
  - The server reacts to the event, handles the message and emits an event on
    'ready:device_mapping_deployment:id' when the resource is ready
  ...
  - A :timeout hits the server, it retries to send the device mapping information to the
    device
  """
  ...
  @doc """
  Starts the provisioner of a device mapping deployment.
  ...
  def provision(device_mapping_deployment, deployment, tenant, opts \\ []) do
    ...
    child_spec = Supervisor.child_spec({__MODULE__, args}, id: device_mapping_deployment.id)
  ...
  @doc """
  Starts a provisioner for a device mapping deployment.
  ...
  - tries to send the deployment information to astarte (calling `&Devices.send_device_mapping_deployment/2`)
  - if an unrecoverable error occurs, broadcasts a failure on the `topic/1` topic.
  - if successful, waits for events on the device mapping deployment itself.

  - if a trigger reaches edgehog, it changes a property in the device mapping deployment resource, emitting an event for the provisioner.
  - the provisioner understands that the device mapping is ready, it then exits successfully.
  ...
  def handle_info(:timeout, %{state: :sent} = state) do
    %{device_mapping_deployment: device_mapping_deployment, tenant: tenant} = state

    device_mapping_deployment = Core.reconcile(device_mapping_deployment, tenant: tenant)

    case device_mapping_deployment do
      :not_found ->
        Core.maybe_send(state)

      {:ok, device_mapping_deployment} ->
        # Reconciliation updated the device_mapping deployment, just update the state as a
  ...
  # We get the device mapping deployment from the broadcast, which is in the :payload ->
  ...
  def handle_info(
    ...
    # Somewhere the device mapping has been marked in some state (pulled/unpulled). For
    # now we can just shutdown gracefully
    {:stop, :normal, new_state}
  end

  # The device mapping deployment provisioning deadline was hit, give up and
  # broadcast a failure so that the orchestrator can react
  @impl GenServer
  def handle_info(:give_up, state) do
    Logger.warning("""
    Device mapping deployment #{state.device_mapping_deployment.id} provisioning timed out. Giving up.
    """)
  ...
  def terminate(:normal, state) do
    %{
      device_mapping_deployment: %{id: id, device_id: device_id} = device_mapping_deployment,
      retries: retries
    } = state
  ...
  @impl GenServer
  def terminate({:shutdown, reason}, %{device_mapping_deployment: device_mapping_deployment})
      when reason in [:max_retries, :device_offline, :timeout_hit] do
    %{id: id, device_id: device_id} = device_mapping_deployment

    Logger.info("""
    Provisioner for device mapping deployment #{id} gave up with reason #{inspect(reason)}.
    """)
    ...
    Phoenix.PubSub.broadcast(
      Edgehog.PubSub,
      "ready:device_mapping_deployments:#{id}",
      {:failure, device_mapping_deployment}
    )
