...
defmodule Edgehog.Containers.Network.Executor do
  @moduledoc false
  ...
  def handle_event(:enter, _old_state, :initialization, data) do
    ...
    Logger.info("Network Deployment #{deployment.id}: entering :initialization state")
  ...
  def handle_event(:internal, :init_device, :initialization, data) do
    ...
    try do
      deployment = Ash.load!(deployment, [:device, :network])

      _ =
        Devices.send_create_network_request!(deployment.device, deployment.network, tenant: tenant)

      _ = Containers.network_deployment_sent!(deployment, tenant: tenant)
  ...
  def handle_event(:enter, _old_state, :sent, data) do
    ...
    Logger.info("Network Deployment #{deployment.id}: entering :sent state")
  ...
  def handle_event(:info, :available, :sent, data) do
    ...
    try do
      _ = Containers.network_deployment_unavailable!(deployment, tenant: tenant)
