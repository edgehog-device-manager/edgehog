...
defmodule Edgehog.Containers.Network.Deployment.Provisioner do
  ...
  - The server subscribes to events on 'network_deployments:id' (id of the network
    deployment)
  ...
  # Send the network if the number of retries does not exceed the max number of
  # retries.
  ...
  # Sends the network deployment, without asking any questions. To check for
  ...
  defp maybe_update_state_on_send(error, state) do
    ...
    Logger.warning(
      "Error while sending the network deployment #{id}: #{inspect(error)}. The operation will be retried shortly."
    )
