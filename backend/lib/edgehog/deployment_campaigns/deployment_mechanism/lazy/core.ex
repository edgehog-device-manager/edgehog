...
defmodule Edgehog.DeploymentCampaigns.DeploymentMechanism.Lazy.Core do
  ...
  def do_operation(target, release, deployment_mechanism, operation_type) do
    case operation_type do
      ...
      :delete ->
        delete(target, release, deployment_mechanism)
  ...
  @doc """
  Deletes the deployment of teh release to the target using the specified deployment mechanism.
  ...
  ## Returns
    ...
    - `{:error, reason}` for any other errors.
  """
  def delete(target, release, _mechanism) do
    if application_deployed?(target, release) do
      {:ok, updated_target} = link_target_deployment(target, release)
