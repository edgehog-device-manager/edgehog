...
defmodule Edgehog.Tenants.Reconciler do
  ...
  defp start_reconciliation_task(%Tenant{} = tenant, tenant_to_trigger_url_fun) do
    Task.Supervisor.start_child(TaskSupervisor, fn ->
      ...
      reconcile_policies_and_triggers(rm_client, trigger_url)
    end)
  end

  defp reconcile_policies_and_triggers(rm_client, trigger_url) do
    with {:ok, version} <- Core.fetch_astarte_version(rm_client) do
      ...
      if supports_policies?,
        do: reconcile_policies(rm_client),
          ...
          Logger.warning("Skipping trigger delivery policies reconciliation. Astarte version does not support policies.")

      # reconcile triggers, pass down policies support and astarte version
      reconcile_triggers(rm_client, trigger_url, supports_policies?, version)
    end
  end
  ...
  defp reconcile_triggers(rm_client, trigger_url, policies_support, astarte_version) do
    trigger_url
    |> Core.list_required_triggers(policies_support)
    |> Enum.each(&Core.reconcile_trigger!(rm_client, &1, astarte_version))
  end
