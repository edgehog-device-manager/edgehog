...
defmodule Edgehog.Tenants.Reconciler.Core do
  ...
  def reconcile_trigger!(%Client.RealmManagement{} = client, required_trigger, astarte_version) do
    if trigger_compatible?(astarte_version, required_trigger),
      do: do_reconcile_trigger(client, required_trigger),
      else:
        Logger.warning(
          "Skipping reconciliation for trigger #{required_trigger["name"]}, astarte minimum requirement not met."
        )
  end
